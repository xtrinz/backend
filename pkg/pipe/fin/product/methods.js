const { ObjectId } = require('mongodb')
    ...
    , rinse        = require('../../../tool/rinse/product')
    , Log          = require('../../../sys/log')
...
const Add      = async function (data_)
{
    const key     =
    { 
          SellerID : ObjectId(data_.SellerID)
        , Name    : data_.Name
    }
    ...
    if (product) 
    {
        console.log('name-already-in-client', { Product: product })
...
const Read      = async function (product_id, entity_)
{
    ...
    if(entity_.Mode === Model.mode.Client)
...
const List           = async function (in_, entity_)
{
    ...
    if(entity_.Mode === Model.mode.Client) in_.Query.IsAvailable = true
    ...
    if(in_.SellerID)  in_.Query.SellerID  = ObjectId(in_.SellerID)
...
const Modify      = async function (data)
{
    ...
    prod.Location      = data.Seller.Address.Location
