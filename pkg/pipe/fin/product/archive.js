const { ObjectId }  = require('mongodb')
    ...
    , project       = require('../../../tool/project/product')
    , Log           = require('../../../sys/log')
...
const ReadAll         = async function (data, mode_)
{
    ...
    Log('find-all-product-by-seller-id', { Data: data, Mode: mode_ })    
    ...
    if(mode_ === Model.mode.Client) query_.Quantity = { $gt : 0 }
...
const UpdateMany = async function (seller_id, data)
{
    Log('update-products-in-a-seller', { SellerID: seller_id, Data: data })
    let qry_ = { SellerID : ObjectId(seller_id) }
      , op   = { $set: data }
    ...
    Log('products-updated', { SellerId: seller_id, Data: data })
}
...
const Remove      = async function (data)
{
    const query_ = 
    {
        SellerID : ObjectId(data.Seller._id), 
