...
const {
  ...
  updateUserPassword,
} = require("../../database/customer/profile");
const validator = require("../../validators/customer/profile");
...
router.post("/pe/verifyuser", validator.pe_pass, async (req, res, next) => {
  try {
...
router.post("/password", validator.update_pass, async (req, res, next) => {
  try {
    ...
    return res.status(code.OK).json("Password updated success fully");
  } catch (error) {
    next(error);
  }
});
