const { verb, method, task, states, status } = require('../../../sys/models')
...
module.exports =
{
    [verb.register]     :
    {
      [method.post]     :
      {
        ...
        , 'body.SellerID'            : [ [ 'requiredIf', 'body.Task', task.Approve ], 'mongoId']
  ...
  , [verb.list]                 :
    {
      [method.get]              :
      {
          'query'                 : [ 'required', 'object' ]
        , 'headers'               : [ 'required', 'object' ]
        , 'query.MobileNo'        : [ 'string', [ 'length', 15, 2 ] ] // [ 'regex', '/^+91[0-9]{10}$/' ]        
        , 'query.Longitude'       : [ 'numeric', [ 'between', -180, 180 ] ]
        , 'query.Latitude'        : [ 'numeric', [ 'between', -90, 90 ] ]
        , 'query.State'           : [ 'string', [ 'in', states.ToBeApproved, states.ToBeCorrected, states.Registered] ]
        , 'query.Status'          : [ 'string', [ 'in', states.Running, states.Closed] ]
